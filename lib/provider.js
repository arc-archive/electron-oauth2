...
export class IdentityProvider {
  ...
  constructor(id, oauthConfig) {
    ...
    this._sessionErrorListener = this._sessionErrorListener.bind(this);
  ...
  _startSession() {
    ...
    this._session.webRequest.onErrorOccurred(this._sessionErrorListener);
  }
  ...
  launchWebAuthFlow(opts={}) {
    this.tokenInfo = undefined;
    this._type = opts.grantType || this.oauthConfig.grantType;
  ...
  _getCustomGrantBody(settings) {
    ...
    const cs = settings.clientSecret || cnf.clientSecret;
    const scopes = settings.scopes || this.oauthConfig.scopes;
    let type = settings.grantType || this.oauthConfig.grantType;
  ...
  _sessionErrorListener(detail) {
    ...
    if (url.indexOf(rUri) === 0) {
      ...
      this._reportOAuthError(new AuthError(error, 'auth_error', this._state));
    }
  }
