...
/** @typedef {import('./provider').AuthCustomData} AuthCustomData */
/** @typedef {import('./provider').TokenCustomData} TokenCustomData */
/** @typedef {import('./provider').CodeCustomData} CodeCustomData */
...
export class IdentityProvider {
   ...
   * @param {AuthorizationOptions=} oauthConfig OAuth2 configuration.
   */
  ...
  clearCache() {
    if (this.tokentStore.has(this.cacheKey)) {
      this.tokentStore.delete(this.cacheKey);
    }
    this.tokenInfo = undefined;
  }
  ...
  launchWebAuthFlow(opts) {
    ...
    this.tokenInfo = undefined;
    this._type = opts.type || opts.response_type ||
      this.oauthConfig.response_type;
   ...
   * @param {AuthorizationOptions} settings Settings passed to the authorize function.
   ...
   * @param {AuthorizationOptions} settings Passed user settings
  ...
  _processPopupResponseData(oauthParams) {
    ...
    if (!state) {
      ...
      this._reportOAuthError(new AuthError(i18n.__('ERR_STATE_MISSMATCH'), 'invalid_state', this._state));
   ...
   * @param {AuthorizationOptions} settings Initial settings object.
   ...
   * @param {AuthorizationOptions} settings Settings object passed to the `authorize()`
   * function
   ...
   * @param {TokenCustomData} data `customData.[type]` property from the settings object.
   ...
   * @param {AuthCustomData} data Value of settings' `customData` property
   */
   ...
   *
   * @param {AuthorizationOptions} settings The same settings as passed to `authorize()`
   * function.
   ...
   *
   * @param {AuthorizationOptions} settings Settings object as for `authorize()` function.
   * @return {Promise<TokenInfo>} Promise resolved to a token info object.
   */
  ...
  _getCustomGrantBody(settings) {
    ...
    const cs = settings.clientSecret || cnf.client_secret;
    const scopes = settings.scopes || this.oauthConfig.scopes;
    let type = settings.type || settings.response_type ||
      this.oauthConfig.response_type;
  ...
  computeExpires(tokenInfo) {
    ...
    tokenInfo.expires_at = expiresAt;
