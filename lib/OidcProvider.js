...
export class OidcProvider extends IdentityProvider {
  ...
  async getAuthTokens(settings={}) {
    const config = { ...this.oauthConfig, ...settings };
    this[settingsValue] = config;
    this.checkConfig();
    try {
      const list = await this.getTokensInfo();
      if (list) {
        const { scopes } = config;
        // @ts-ignore
        const hasUnauthorized = list.some((t) => this.isTokenAuthorized(t, scopes));
        if (!hasUnauthorized) {
          list.forEach((t) => {
      ...
      return this.authorizeOidc(config);
    } catch (cause) {
   ...
   *
   * @return {Promise<OidcTokenInfo[]>} Cached tokens or `undefined` if there's
   * no cached tokens or cached tokens expired.
   */
  async getTokensInfo() {
    const list = await this.restoreTokensInfo();
    if (!Array.isArray(list) || !list.length) {
      return;
    }
    const valid = /** @type OidcTokenInfo[] */ (list.filter((i) => !this.isExpiredOidcToken(i)));
    if (!valid.length) {
      return;
    }
    return valid;
   ...
   * @return {boolean}
   */
  isExpiredOidcToken(token) {
    if (!token) {
      return true;
    }
    const error = /** @type OidcTokenError */ (token);
    if (error.error) {
      return true;
    }
    const typed = /** @type OidcTokenInfo */ (token);
    const { time, expiresIn } = typed;
    if (!expiresIn || !time) {
      return false;
    }
    return (time + (expiresIn * 1000)) < Date.now();
  }

  /**
   * Restores authorization token information from the local store.
   *
   * @return {Promise<(OidcTokenInfo|OidcTokenError)[]>} Token info object or `undefined` if not set or expired.
   */
  async restoreTokensInfo() {
    ...
    return /** @type {(OidcTokenInfo|OidcTokenError)[]} */ (data);
  }

  /**
   * Caches token data in local storage.
   *
   * @param {(OidcTokenInfo|OidcTokenError)[]} tokenInfo
   * @return {Promise<void>} Resolved promise when the code is executed
   */
  async storeTokens(tokenInfo) {
