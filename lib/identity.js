...
export class Oauth2Identity {
   ...
   *
   * @param {Object} e
   * @param {Object} options Oauth options. See
   * `Oauth2Identity.launchWebAuthFlow` for description
   ...
   * - `token_uri` or `accessTokenUri` {String} Optional, required if
   * `response_type` is code
   * - `redirect_uri` or `redirectUri` {String} Auth redirect URI
   * - `client_secret` or `clientSecret` {String} Optional, required if
   * `response_type` is code
  ...
  static async getAuthToken(opts) {
    ...
    return await provider.getAuthToken(opts);
  }
  /**
   * Removes cached token info.
   *
   * @param {?Object} opts When provided it is the same as for
  ...
  static async removeToken(opts) {
    if (!opts) {
      opts = await Oauth2Identity.getOAuthConfig();
    }
