...
class Oauth2Identity {
   ...
   * @param {String} id Id generated in the renderer to recognize the request.
   */
  static _removeTokenHandler(e, options, id) {
    Oauth2Identity.removeToken(options)
    .then(() => {
   ...
   *
   * @param {?Object} opts When provided it is the same as for `launchWebAuthFlow()`
   * function. When not set it reads `package.json` object for oauth2 configuration.
   * @return {Promise}
   */
  static removeToken(opts) {
    const p = opts ? Promise.resolve(opts) : Oauth2Identity.getOAuthConfig();
    return p
    .then((config) => {
      return Oauth2Identity._getOrCreateProvider(config);
    })
    .then((provider) => provider.clearCache());
  }
  /**
